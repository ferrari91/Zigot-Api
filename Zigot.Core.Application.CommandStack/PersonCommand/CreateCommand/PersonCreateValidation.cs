...
namespace Zigot.Core.Application.CommandStack.PersonCommand.CreateCommand
{
    public class PersonCreateValidation : AbstractValidator<PersonCreateRequest>
    {
        ...
        private void ValidateParentsNames()
        {
        ...
        {
            var person = await _personRepository.FindAsync(x => x.FullName.ToLower() == fullName.ToLower(), cancellationToken);
            return person == null;
        }
    }
